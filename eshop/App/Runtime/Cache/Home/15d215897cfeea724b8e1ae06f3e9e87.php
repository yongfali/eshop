...
<script type="text/javascript">
  var cancleComplain = "<?php echo U('Home/UserMsgManage/Withdrawal');?>";
</script>
</head>
<body>
<!-- 头部header开始 -->
              ...
              <?php if(empty($lists)): ?><tr>
                ...
                <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td class="orders_action">
                      <a href="<?php echo U('Home/UserMsgManage/complaintDetail',array('cid' => $item['id']));?>">详情</a>
                      <?php if($item["is_cancle"] == 0): ?><a href="javascript:void(0);" onclick="Withdrawal(<?php echo ($item['orderid']); ?>)">撤诉</a><?php endif; ?>
                    </td>
