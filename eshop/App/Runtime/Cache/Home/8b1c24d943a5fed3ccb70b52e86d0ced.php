...
<div class="scenter_left">
  ...
  <div class="scenter_right">
    ...
    <form id="goodEdit" autocomplete="off" rel="<?php echo U('eidtSave');?>">
      <!-- 商品信息 -->
      <div class="scenter_right_content" id="nav1" >
        <table class="goodAdd_table">
          <tbody>
            ...
            <tr>
              ...
              <td><input type="text" class="inputs" name="goodsName" value="<?php echo ($goodInfo["name"]); ?>"/></td>
              <td rowspan="6" valign="top">
                <div id="preview" class="good_photo">
                  <img id="imghead" border="0" src="/eshop/<?php echo ($goodInfo["good_log"]); ?>">
                </div>
            ...
            <tr>
              ...
              <td><input type="text" class="inputs" name="marketPrice" value="<?php echo ($goodInfo["marketprice"]); ?>"/></td>
            </tr>
            <tr>
              <td class="itemsName"><span class="icon">*</span>店铺价格：</td>
              <td><input type="text" class="inputs" name="shopPrice" value="<?php echo ($goodInfo["shopprice"]); ?>"/></td>
            </tr>
            <tr>
              <td class="itemsName"><span class="icon">*</span>商品库存：</td>
              <td><input type="text" class="inputs" name="stock" value="<?php echo ($goodInfo["stock"]); ?>" id="stock"/></td>
            </tr>
            <tr>
              <td class="itemsName">预警库存：</td>
              <td><input type="text" class="inputs" name="warnStock" value="<?php echo ($goodInfo["stock_warning"]); ?>"/></td>
            </tr>
            <tr>
              <td class="itemsName"><span class="icon">*</span>商品产地：</td>
              <td><input type="text" class="inputs" name="place" value="<?php echo ($goodInfo["place"]); ?>"/></td>
            </tr>
            <tr>
              <td class="itemsName"><span class="icon">*</span>商品状态：</td>
              <td class="attrSelect"><input type="radio"  name="status" value="1" <?php if($goodInfo["status"] == 1): ?>checked="checked"<?php endif; ?> />
                <span>上架</span>
                <input type="radio"  name="goodStatus" value="0" <?php if($goodInfo["status"] == 0): ?>checked="checked"<?php endif; ?>/>
                <span>下架</span></td>
            </tr>
            <tr>
              <td class="itemsName">商品属性：</td>
              <td class="attrSelect">
              	<input type="checkbox"  name="recomend" value="1" id="recomend"<?php if($goodInfo["is_recomend"] == 1): ?>checked="checked"<?php endif; ?> />
              	<span>推荐</span>
              	<input type="checkbox"  name="hot" value="1" <?php if($goodInfo["is_hot"] == 1): ?>checked="checked"<?php endif; ?> />
              	<span>热销</span>
              	<input type="checkbox"  name="newgood" value="1" <?php if($goodInfo["is_new"] == 1): ?>checked="checked"<?php endif; ?>/>
              	<span>新品</span>
              </td>
            </tr>
            <tr>
              ...
              <td class="attrSelect" width="500px"><select name="wfirst" id="wfirst" onchange="getNextCart('wSecond',this,0)">
                  ...
                  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><option value="<?php echo ($data["id"]); ?>" <?php if($navName['w_fid'][0]['name'] == $data["name"] ): ?>selected="selected"<?php endif; ?> ><?php echo ($data["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
                <select name="wSecond" id="wSecond" onchange="getNextCart('wThird',this,1)">
                  <?php if(empty($navName['w_sid'][0]['name'])): ?><option value="0">--请选择--</option>
                    <?php else: ?>
                    <option value="<?php echo ($navName['w_sid'][0]['w_sid']); ?>"><?php echo ($navName['w_sid'][0]['name']); ?></option><?php endif; ?>
                </select>
                <select name="wThird" id="wThird">
                  <?php if(empty($navName['w_tid'][0]['name'])): ?><option value="0">--请选择--</option>
                    <?php else: ?>
                    <option value="<?php echo ($navName['w_tid'][0]['w_tid']); ?>"><?php echo ($navName['w_tid'][0]['name']); ?></option><?php endif; ?>
                </select>
              </td>
            </tr>
            <tr>
              ...
              <td class="attrSelect">
                <select name="sfirst" onchange="getShopCart(this)" action="<?php echo U('getShopCart');?>" id="sfirst">
                  <option value="0">--请选择--</option>
                  <?php if(is_array($list1)): $i = 0; $__LIST__ = $list1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><option value="<?php echo ($item["id"]); ?>" 
                   <?php if($navName['s_fid'][0]['name'] == $item["name"] ): ?>selected="selected"<?php endif; ?> ><?php echo ($item["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
                <select name="sSecond" id="sSecond">
                  <?php if(empty($navName['s_sid'][0]['sname'])): ?><option value="">--请选择--</option>
                    <?php else: ?>
                    <option value="<?php echo ($navName['s_sid'][0]['s_sid']); ?>"><?php echo ($navName['s_sid'][0]['sname']); ?></option><?php endif; ?>
                </select>
              </td>
            </tr>
            <tr>
        ...
        <input type="hidden" name="goodId" value="<?php echo ($goodInfo["id"]); ?>"/>
      ...
      <div class="scenter_right_content" id="nav2" style="display:none" >
        <table class="goodAdd_table">
          <tbody>
            ...
            <?php if(is_array($goodLable)): $i = 0; $__LIST__ = $goodLable;if( count($__LIST__)==0 ) : echo "该商品咱没属性" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i; if($item["name"] == '是否合格' ): ?><tr>
                <td class="itemsName1"><?php echo ($item["name"]); ?>：</td>
                <td class="attrSelect"><input type="radio"  name="pro[<?php echo ($item["lableid"]); ?>]" value="1" <?php if($item["lablecontent"] == 1): ?>checked="checked"<?php endif; ?>/>
                  <span>是</span>
                  <input type="radio"  name="pro[<?php echo ($item["lableid"]); ?>]" value="0"<?php if($item["lablecontent"] == 0): ?>checked="checked"<?php endif; ?>/>
                  <span>否</span></td>
                </tr>
                <?php elseif($item["name"] == '是否绿色食品'): ?>
                <tr>
                  <td class="itemsName1"><?php echo ($item["name"]); ?>：</td>
                  <td class="attrSelect"><input type="radio"  name="pro[<?php echo ($item["lableid"]); ?>]" value="1" <?php if($item["lablecontent"] == 1): ?>checked="checked"<?php endif; ?>/>
                    <span>是</span>
                  ...
                  <tr>
                    <td class="itemsName1">
                      <?php switch($item["name"]): case "净含量": case "出产日期": case "保质期": ?><span class="icon">*</span><?php break;?>
                        <?php default: endswitch;?>
                      <?php echo ($item["name"]); ?>：
                    </td>
                    <td><input type="text" class="inputs" name="pro[<?php echo ($item["lableid"]); ?>]" value="<?php echo ($item["lablecontent"]); ?>"/></td>
                  </tr><?php endif; endforeach; endif; else: echo "该商品咱没属性" ;endif; ?>
          </tbody>
        </table>
        <input type="submit" value="保存" class="btn" style="margin-left:150px"/>
         <input type="reset" value="重置" class="btn"/>
      ...
      <div class="scenter_right_content" id="nav3" style="display:none" > 
      	<div class="uploader-list-container">
      		<div class="queueList">
      			<div id="dndArea" class="placeholder <?php if(!empty($goodImgs)): ?>element-invisible<?php endif; ?>" >
      				<div id="filePicker-2"></div>
      				<p>或将照片拖到这里，单次最多可选10张，每张图片的大小不要超过5M（图片格式包含jpe,jpeg,phg,gif）</p>
      			</div>
      			<ul class="filelist">
      				<?php if(is_array($goodImgs)): $i = 0; $__LIST__ = $goodImgs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><li class="state-complete">
      						<p class="title"></p>
      						<p class="imgWrap">
      							<img src="/eshop/<?php echo ($item["img"]); ?>">
      						</p>
      						<span class="success1"></span>
      						<span class="delImg">
      							<a href="javascript:;" onclick="delImg(this,1)" value="<?php echo ($item["id"]); ?>">删除</a>
      						</span>
      					</li><?php endforeach; endif; else: echo "" ;endif; ?>
      			</ul>
      		</div>
      		<div class="statusBar" style="<?php if(empty($goodImgs)): ?>display:none;<?php endif; ?>">
