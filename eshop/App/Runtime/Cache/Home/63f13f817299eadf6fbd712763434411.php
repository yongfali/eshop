...
<div class="scenter_left">
  ...
  <div class="scenter_right">
    <div class="scenter_right_title"> <span>待发货订单</span> 
    </div>
    <div class="scenter_right_content"> 
      ...
      <form action=" " id="order_search">
        <div class="auth_good_search">
          ...
          <input type="button" value="查询"  class="btn" id="order_select" types="waitDelivery"/>
        </div>
      </form>
      <!-- 头部检索框 -->  
      <!-- 待付款订单内容展示 -->
        ...
        <div id="Orders_List_Contetn">
          <table class="orders_table">
            ...
            <?php if(empty($lists)): ?><tbody>
                <tr style="line-height:36px;text-align:center; color:red;"><td colspan="6">暂没有待发货订单！！！</td></tr>
              </tbody>
              <?php else: ?>
              <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tbody class="orders_items">
                  <tr class="orders_head">
                    <td colspan="6">
                      <div class="order_time">
                        ...
                        <span>下单时间：<?php echo (date("Y-m-d  H：i：s",$item["create_time"])); ?></span>
                      </div>
                      <div class="order_status">等待发货</div>
                    </td>
                  </tr>
                  <tr class="orders_info">
                    ...
                    <td class="orders_action">
                      <a href="javascript:void(0)" onclick="orderDeliver(<?php echo ($item['orderid']); ?>)">发货</a>
