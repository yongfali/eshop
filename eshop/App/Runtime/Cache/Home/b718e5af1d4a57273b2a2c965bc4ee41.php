...
<div class="ucenter_left">
  ...
  <div class="ucenter_right">
    <div class="ucenter_right_title"> <span>地址管理</span> </div>
    <div class="ucenter_right_content">
      <div class="uaccout">
        <label>登录账号：</label>
        <span><?php echo (session('uname')); ?></span><span style="font-size:15px;color:#3F11F8;">（您已经拥有了<strong style="color:red"><?php echo ($num); ?></strong>个地址最多可以添加5个地址）</span> </div>
      <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><table class="uaddress_table">
          <tbody>
            <tr>
              <td>收货人基本信息</td>
              <td><?php if($data["is_first"] == 1): ?><span style="color:red">默认地址</span><?php endif; ?>
                <a href="javascript:delAddr(<?php echo ($data["id"]); ?>);" class="addr_del" id="addr_del" > <img src="/eshop/Public/Home/Icon/delete1.png" /> </a></td>
            </tr>
            <tr>
              <td class="addr-title">姓名：</td>
              <td><?php echo ($data["username"]); ?></td>
            </tr>
            <tr>
              <td class="addr-title">邮编：</td>
              <td><?php echo ($data["postcode"]); ?></td>
            </tr>
            <tr>
              <td class="addr-title">所在地区：</td>
              <td><?php echo formatAddr($data['location'],0);?></td>
            </tr>
            <tr>
              <td class="addr-title">地址：</td>
              <td><?php echo ($data["streetinfo"]); ?></td>
            </tr>
            <tr>
              <td class="addr-title">联系电话：</td>
              <td><?php echo ($data["tel"]); ?></td>
            </tr>
            <tr>
              <td class="addr_edit" colspan="2"><a href="javascript:;" class="modifyAddr" value="<?php echo ($data["id"]); ?>" >编辑</a>
                <?php if($data["is_first"] != 1): ?><a href="javascript:setDefaultAddr(<?php echo ($data["id"]); ?>);">设为默认</a><?php endif; ?></td>
            </tr>
          </tbody>
        </table><?php endforeach; endif; else: echo "" ;endif; ?>
      <div class="add_addr_btn">
        <?php if($num <= 4 ): ?><input type="button"  value="新增收货地址" id="addressEdit"/><?php endif; ?>
      </div>
    </div>
  </div>
  <!-- 用户中心右侧内容显示结束 --> 
