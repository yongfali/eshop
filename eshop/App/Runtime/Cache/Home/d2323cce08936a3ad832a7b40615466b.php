...
<div class="moreInfo_wrapper">
  ...
  <!-- 左侧导航标签 -->
  <div  class="moreInfo_wrapper_right">
    <div class="moreInfo_right_head"> 
      <span>
        <?php switch($infoType): case "info": ?>资讯列表<?php break;?>
          <?php case "discount": ?>优惠活动列表<?php break;?>
          <?php default: ?>公告列表<?php endswitch;?>
      </span> 
    </div>
    <!-- 标题 -->
        ...
        <ul>
          <?php if(empty($infoList)): ?><li>暂无信息！！！</li>
            ...
            <?php if(is_array($infoList)): $i = 0; $__LIST__ = $infoList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><li>
                <a href="<?php echo U('Home/Index/infoDetail',array('id' => $data['id']));?>"><?php echo ($data["title"]); ?></a>
