...
<!-- header结束 -->
	<div class="orderPay_wrapper">
		<div class="shopcart_nav">
			...
			<dl class="third doing">
				<dt class="cart_num">3</dt>
				<dd class="cart_text">付款（完成订单）</dd>
			</dl>
		</div>
		<div class="orderPay_content">
			<div class="s_submit">
				<img src="/eshop/Public/Home/OrderImage/order_submit.jpg">
			</div>
			<div class="order_info">
				订单号：
				<?php if(!empty($orderNumber)): ?><span>
						<?php if(is_array($orderNumber)): $i = 0; $__LIST__ = $orderNumber;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i; echo ($item); ?>，<?php endforeach; endif; else: echo "" ;endif; ?>
					</span><?php endif; ?>
				消费金额：<span>&yen;<?php echo ($totalPrice); ?></span>
			</div>
			<div class="pay_left">
				<form　action="" method="post" id="pay" autocomplete="off">
					<div class="left_items"><label>账号：</label><input type="text" placeholder="acount" /></div>
					<div class="left_items"><label>密码：</label><input type="password" placeholder="password" /></div>
					<div class="left_items1"><button>确认付款</button></div>
				</form>	
			</div>
			<div class="pay_right">
				<img src="/eshop/Public/Home/Image/float_icon_weixin.jpg">
			</div>
			<div class="continue_shoping">
				<a href="<?php echo U('home/Index/index');?>">继续购物</a>
			</div>
		</div><!-- 支付页面包裹内容结束 -->
		<!-- 导航标题结束 -->
	</div><!-- 支付页面中间结束 --> 
